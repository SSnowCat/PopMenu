...
final public class PopMenuViewController: UIViewController {
    ...
    public var isChonsen: Bool = false
    ...
    public convenience init(sourceView: AnyObject? = nil, actions: [PopMenuAction],chonsenState:Bool? = false,currentIndex:Int? = 0, appearance: PopMenuAppearance? = nil) {
        self.init(nibName: nil, bundle: nil)
        
        self.sourceView = sourceView
        self.actions = actions
        self.currentIndex = currentIndex
        self.isChonsen = chonsenState!
...
extension PopMenuViewController {
    ...
    fileprivate func configureActionsView() {
        ...
        if isChonsen {
            actions[currentIndex!].isCurrentIndex = true
        } else {
            actions.forEach { action in
                    action.isCurrentIndex = true
                }
        }
...
//            action. = 0
//            action as! PopMenuDefaultAction
